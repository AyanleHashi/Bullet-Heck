...
class BasicEnemy {
    ...
    public void takeDamage() {
        for (int i=0;i<p.projectiles.size();i++) {
            if ((xPos-(p.attack+3) < p.projectiles.get(i).get(0) && p.projectiles.get(i).get(0) < xPos+(p.attack+3)) &&
                    (yPos-(p.attack+3) < p.projectiles.get(i).get(1) && p.projectiles.get(i).get(1) < (yPos+p.attack+3))) {
